...
- 🤖 **智能对话** - LangChain 多轮对话 + 流式输出，SSE 流式响应
- 📚 **RAG 问答** - 向量检索增强（召回 → 重排 → TopK），支持文档上传、自动建索引、自动更新知识库，含上下文自动压缩
- 🔧 **AIOps 诊断** - Plan-Execute-Replan 自动故障诊断和根因分析
- 🧩 **Skill 业务技能系统** - 将标准业务流程固化为固定执行步骤，命中后严格执行，不走自由规划
- 🔌 **MCP 集成** - 日志查询（CLS）和监控数据（Monitor）工具接入，带重试拦截器
- 📈 **Prometheus 告警联动** - 诊断时自动注入当前活跃告警，按告警名命中对应处置技能

...
async def executor(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    try:
        ...
        if state.get("skill_context"):
            # Skill 模式：注入完整技能定义。当前步骤是技能流程中的一环，
            # 严格按流程规定的方法、工具和参数执行（分支判断参照流程中的分支章节）。
            system_content = f"""你是一个能力强大的助手，负责执行具体的任务步骤。
...
当前任务属于标准处理流程（{state.get('skill_name', '')}），以下是该流程的完整定义。
请严格按流程中规定的方法和参数执行当前步骤：

{state['skill_context']}"""
        else:
            system_content = """你是一个能力强大的助手，负责执行具体的任务步骤。

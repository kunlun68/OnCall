...
async def replanner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    # Skill 模式：规范流程不可改序，禁止 replan（调整计划会偏离标准流程）。
    # 剩余步骤按 skill 顺序继续执行；全部执行完毕后生成最终响应。
    # skill 内部的分支决策由 Executor 每步结合 skill 的分支章节判断，不在此处自由 replan。
    if state.get("skill_name"):
        logger.info(f"Skill 模式（{state['skill_name']}）：严格按固定步骤执行，跳过 replan 决策")
        if plan:
        ...
        return await _generate_response(state, skill_llm)  # respond：步骤已执行完毕

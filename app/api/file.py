...
from app.services.document_parser_service import SUPPORTED_EXTENSIONS
...
# 支持的文件类型（带点、小写）——从解析层统一 import，避免多份名单漏改
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if f".{file_extension}" not in SUPPORTED_EXTENSIONS:
            # 老版 .doc 格式单独给出可操作提示（python-docx 只认 .docx）
            if file_extension == "doc":
                raise HTTPException(
                    status_code=400,
                    detail=f"不支持老版 .doc 格式，请在 Word 中另存为 .docx 后重新上传。",
                )
            raise HTTPException(
                status_code=400,
                detail=f"不支持的文件格式，仅支持: {SUPPORTED_EXTENSION_NAMES}",
            )

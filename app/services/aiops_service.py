...
def _fetch_active_alert_hint() -> str:
    """获取当前活跃告警名，拼成任务提示文本，供 Planner 按告警名命中业务技能。

    Prometheus 不可用或暂无告警时返回空字符串（静默降级，走 RAG 兜底）。
    """
    try:
        ...
        if err:
            logger.warning(f"获取活跃告警失败（skill 匹配降级为 RAG 兜底）: {err}")
            return ""

        alerts = (body.get("data") or {}).get("alerts") or []
        names = sorted(
            {
                str((a.get("labels") or {}).get("alertname") or "").strip()
                for a in alerts
        ...
        logger.info(f"诊断任务注入活跃告警: {', '.join(names)}")
        return f"\n\n当前活跃告警: {', '.join(names)}"

    except Exception as e:
        logger.warning(f"获取活跃告警异常（skill 匹配降级为 RAG 兜底）: {e}")
        return ""
...
class AIOpsService:
    ...
    async def diagnose(
        ...
        # 注入当前活跃告警名，使 Planner 能按告警名命中对应的业务技能（Skill）。
        # 失败或暂无告警时 alert_hint 为空，走原有知识库 RAG 兜底。
        alert_hint = _fetch_active_alert_hint()
        if alert_hint:
            aiops_task += alert_hint

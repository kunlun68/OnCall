...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            # 获取所有支持的文件（遍历目录，按扩展名过滤，
            # 兼容大写扩展名如 .MD；支持列表以 parser 的 SUPPORTED_EXTENSIONS 为唯一来源）
            files = [
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            # 1. 提取文件内容（按扩展名自动匹配处理器：txt/md 直读、pdf 用 pypdf、docx 用 python-docx）
            content = document_parser_service.extract_text(str(path))
            logger.info(f"提取文件内容: {path}, 内容长度: {len(content)} 字符")
